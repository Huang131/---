...
@Author: Huang
@dev: python3 vscode
@Date: 2019-11-12 23:23:21
@LastEditTime: 2019-11-12 23:39:58
@FilePath: \\机器学习实战\\15-MapReduce\\mrSVM.py
'''
...
class MRsvm(MRJob):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.data = pickle.load(open(r'15-MapReduce\svmDat27'))
        self.w = 0
        self.eta = 0.69
        self.dataList = []
    ...
    def configure_options(self):
        ...
        self.add_passthrough_option('--batchsize',
                                    dest='batchsize',
                                    default=100,
                                    type='int',
    ...
    def map_fin(self):
        ...
        if self.w == 0:
            self.w = [0.001] * np.shape(X)[1]  # init w on first iteration
        for index in self.dataList:
            p = np.mat(self.w) * X[index, :].T  # calc p=w*dataSet[key].T
            if labels[index] * p < 1.0:
                yield (1, ['u',
                           index])  # make sure everything has the same key
    ...
    def reduce(self, _, packedVals):
        ...
        for index in self.dataList:
            wDelta += float(
                labels[index]) * X[index, :]  # wDelta += label*dataSet
        ...
        for mapperNum in range(1, self.numMappers + 1):
            ...
            if self.t < self.options.iterations:
                ...
                # emit random ints for mappers iid
                for j in range(self.k / self.numMappers):
                    yield (mapperNum,
                           ['x',
                            np.random.randint(np.shape(self.data)[0])])

    def steps(self):
        return ([
            self.mr(mapper=self.map,
                    reducer=self.reduce,
                    mapper_final=self.map_fin)
